...
### Revision history
- **v3.1 (2026-06-29):** Post-implementation adversarial re-audit of the *built* code (7 lenses, each finding independently refutation-tested; the cross-contest solvency invariant was attacked and held by construction). One real defect found + fixed: **result-market oracle binding.** `settle_contest` re-derived each result-market PDA and checked owner/`num_buckets`/status — but **not** that the market's `settle_authority` equals the contest's keeper. Because `initialize_market` is permissionless and the result-market PDA is deterministic per `(fixture, RESULT_MARKET_ID)`, an attacker could front-run the keeper, squat the PDA with their own authority, and `settle` each leg to make their own ticket the perfect line. Fix: `settle_contest` now requires `result_market.settle_authority == contest.settle_authority` (§6.3/§9). A squat with a foreign authority is rejected (keeper voids → refunds); a squat naming the keeper as authority can only be settled *by* the keeper (`settle` has `has_one`), yielding the true result. Residual is griefing (a squat forces a void/refund), mitigated operationally by the keeper creating result markets **before** the contest (§10.2/§12); optional mainnet hardening is to permission `initialize_market` for `market_id == RESULT_MARKET_ID`. Also: error enum restored to append-only (a mid-enum `InvalidBucketCount` had shifted parimutuel codes); added settle-side negative tests (non-keeper oracle rejected, zero-stake leg, reserve-fence assertion).
...
- **Winning buckets are not a parameter.** Pass exactly `num_matches` result-market accounts as remaining accounts. For each `i < num_matches`, the program **re-derives** the expected market PDA from `["market", contest.fixtures[i].to_le_bytes(), RESULT_MARKET_ID.to_le_bytes()]`, asserts `remaining_accounts[i].key() == that PDA`, `owner == this program`, `num_buckets == 3`, **`settle_authority == contest.settle_authority`** (the oracle binding, below), and that its status is `Settled` **or** `Voided` (the per-match `settle` records the proof-determined `winning_bucket` even on its zero-winner void — a real result with no stake on the winning side), then **copies** that market's `winning_bucket` into `contest.winning_buckets[i]`. A `Voided` market with **no** recorded bucket (a genuinely abandoned match) yields no bucket → `settle_contest` rejects (`ResultMarketNotSettled`); the keeper voids the contest instead. This binds the result to the card's actual fixtures (a shadow market can't be substituted).
  - **Oracle binding (v3.1).** The PDA re-derivation pins *which* account is read, but `initialize_market` is permissionless, so it does not pin *who* settled it. Requiring `result_market.settle_authority == contest.settle_authority` ensures only results settled by the contest's own keeper are trusted, closing a front-run-squat that could otherwise dictate every leg. A foreign-authority squat is rejected → keeper voids → refunds; a squat that names the keeper as authority can only be settled *by* the keeper (`settle` has `has_one = settle_authority`) → true result. The all-markets-resolved requirement is also the real liveness gate.
...
- Ensure a per-match **result market** exists for each chosen fixture (create if missing) so the verified-settlement path (§9) applies. **Create these result markets _before_ calling `create_contest`** (and settle them with the same keyed authority used as the contest's `settle_authority`): the v3.1 oracle binding requires `result_market.settle_authority == contest.settle_authority`, and creating-first denies an attacker the window to squat the deterministic PDA (§16).
...
- **Oracle binding (v3.1):** a result market settled by a **non-keeper** authority (attacker squat) is rejected (`ResultMarketMismatch`) even though its PDA, owner, and `num_buckets` are valid — only the contest keeper's own settlements are trusted.
...
- **Settlement trust (now mostly closed):** winning buckets are verified from bound result-market PDAs that must be settled by the contest's own keeper (v3.1 oracle binding, §9); `perfect_count` stays keeper-supplied but is blast-radius-capped — framed **verifiable, not trustless**, with the registration-window/merkle hardening designed.
- **Result-market squat (griefing, v3.1):** because `initialize_market` is permissionless, an attacker can squat a card fixture's result-market PDA with a foreign authority; the oracle binding then makes `settle_contest` reject it, forcing the keeper to `void_contest` (full refunds — no theft). Mitigated operationally by the keeper creating result markets **before** publishing the contest (§10.2/§12). Optional on-chain hardening for mainnet: permission `initialize_market` for `market_id == RESULT_MARKET_ID` to a canonical oracle so the slot can't be squatted at all.
