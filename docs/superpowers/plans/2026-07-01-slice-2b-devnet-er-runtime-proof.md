...
- **DEPLOYED to devnet (2026-07-01):** in-place upgrade of the same id `By8y6y…` with the `refund_voided` build. Sig `2G8baDy1gr1b4HwMWa5Th2JMNjsomEM1hp1DuAf83QbiFdZpKZsni8QFvYRv7rpjvs37kJGGbDSd4fs45NjR5kLA`; Data Length **732080→742320**, new slot **473285766**, authority unchanged `FP39zt…`; net **0.075◎** (ProgramData already extended in Step 1 → only a +6304B auto-extend). On-chain IDL **published for the first time** via `anchor idl init` (account `GTk8YMCZ4u5xboLPdPNQS4vhYfQaqrFoh9afcLZtNCRr`, +0.10◎) — a fetch now returns `refund_voided`. The deployed `.so` is byte-identical to the artifact the 11/11 localnet suite exercised; `refund_voided` is a pure base-layer refund (no ER interaction), so the localnet proof covers its on-chain behavior.
